...
static GstStateChangeReturn gst_nb_change_state (GstElement * element,
...
gst_nb_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    ...
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      ...
      OMX_SetParameter (nb_handle, OMX_IndexConfigAudioAmrFrameFormatFSF,
          param);
...
gst_omx_amrnbdec_get_type (void)
{
  ...
  if (G_UNLIKELY (type == 0)) {
    ...
    type =
        g_type_register_static (GST_OMX_BASE_FILTER_TYPE, "GstOmxAmrNbDec",
        type_info, 0);
