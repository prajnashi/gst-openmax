...
settings_changed_cb (GOmxCore * core)
{
...

  {
    ...
    new_caps = gst_caps_new_simple ("video/x-raw-yuv",
        ...
        "format", GST_TYPE_FOURCC, format, NULL);
...
sink_setcaps (GstPad * pad, GstCaps * caps)
{
  ...
  if (!gst_structure_get_fraction (structure, "framerate", &self->framerate_num,
...
gst_omx_base_videodec_get_type (void)
{
  ...
  if (G_UNLIKELY (type == 0)) {
    ...
    type =
        g_type_register_static (GST_OMX_BASE_FILTER_TYPE, "GstOmxBaseVideoDec",
        type_info, 0);
