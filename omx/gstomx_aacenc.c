  ...
  /* Properties stuff */
  {
    ...
    g_object_class_install_property (gobject_class, ARG_PROFILE,
        g_param_spec_enum ("profile", "Enocding profile",
            ...
            GST_TYPE_OMX_AACENC_PROFILE, DEFAULT_PROFILE, G_PARAM_READWRITE));
    ...
    g_object_class_install_property (gobject_class, ARG_OUTPUT_FORMAT,
        g_param_spec_enum ("output-format", "Output format",
            ...
            DEFAULT_OUTPUT_FORMAT, G_PARAM_READWRITE));
  }
}
...
sink_setcaps (GstPad * pad, GstCaps * caps)
{
  ...
  /* Input port configuration. */
  {
    ...
    OMX_SetParameter (omx_base->gomx->omx_handle, OMX_IndexParamAudioPcm,
        param);
...
gst_omx_aacenc_get_type (void)
{
  ...
  if (G_UNLIKELY (type == 0)) {
    ...
    type =
        g_type_register_static (GST_OMX_BASE_FILTER_TYPE, "GstOmxAacEnc",
        type_info, 0);
