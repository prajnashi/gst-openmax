  ...
  /* Properties stuff */
  {
    ...
    g_object_class_install_property (gobject_class, ARG_USE_TIMESTAMPS,
        g_param_spec_boolean ("use-timestamps", "Use timestamps",
            "Whether or not to use timestamps", TRUE, G_PARAM_READWRITE));
  }
}
    ...
    while (G_LIKELY (buffer_offset < GST_BUFFER_SIZE (buf))) {
      ...
      if (G_LIKELY (omx_buffer)) {
        ...
        if (self->use_timestamps) {
          ...
          GST_LOG_OBJECT (self,
              "omx_buffer->nTimeStamp=%" G_GINT64_FORMAT ", buf_time=%"
              GST_TIME_FORMAT, omx_buffer->nTimeStamp,
...
gst_omx_base_filter_get_type (void)
{
  ...
  if (G_UNLIKELY (type == 0)) {
    ...
    type =
        g_type_register_static (GST_TYPE_ELEMENT, "GstOmxBaseFilter", type_info,
        0);
    g_free (type_info);
